...
export function CrosswordBoard({ puzzle }: CrosswordBoardProps) {
    ...
    // Given a cell and direction, find which clue it belongs to (the one whose word covers this cell)
    const getActiveClue = (r: number, c: number, dir: 'across' | 'down') => {
        const clues = dir === 'across' ? puzzle.acrossClues : puzzle.downClues;
        return clues.find(clue => {
            if (dir === 'across') {
                // clue starts at (clue.row, clue.col) and extends right for clue.answer.length cells
                return clue.row === r && clue.col <= c && c < clue.col + clue.answer.length;
            } else {
                return clue.col === c && clue.row <= r && r < clue.row + clue.answer.length;
            }
        }) ?? null;
    };

    const advanceToNextClue = (dir: 'across' | 'down') => {
        const clues = dir === 'across' ? puzzle.acrossClues : puzzle.downClues;
        if (clues.length === 0) return;

        // Find which clue we're currently in
        const current = getActiveClue(selectedCell?.r ?? 0, selectedCell?.c ?? 0, dir);
        if (!current) {
            setSelectedCell({ r: clues[0].row, c: clues[0].col });
            return;
        }
    ...
    const handleKeyDown = (e: React.KeyboardEvent, r: number, c: number) => {
        ...
        if (key === 'Enter' || key === 'Tab') {
            e.preventDefault();
            // Try to step forward in the current word direction first
            if (direction === 'across' && c + 1 < puzzle.grid[r].length && puzzle.grid[r][c + 1] !== '#') {
                ...
                // At end of word — jump to the start of the next clue in the same direction
                advanceToNextClue(direction);
            }
    ...
    return (
        <div className={`w-full flex ${isWideGrid ? 'flex-col' : 'flex-col lg:flex-row'} gap-8 mt-12 animate-in fade-in slide-in-from-bottom-5 duration-700`}>
            ...
            {/* Clues Column */}
            <div className={`w-full ${isWideGrid ? 'flex flex-col' : 'lg:w-96 flex flex-col'} gap-6`}>
                <div className={`flex ${isWideGrid ? 'flex-col md:flex-row' : 'flex-col'} gap-6`}>
                ...
                {showAnswerKey && (
                    <div className="w-full bg-white/80 backdrop-blur-xl border border-purple-200 p-8 rounded-3xl shadow-2xl animate-in fade-in slide-in-from-top-4 duration-500 font-sans">
...
function ClueBox({ title, clues }: { title: string, clues: PlacedWord[] }) {
    // hints: cache of fetched hint text keyed by clue number (never deleted once fetched)
    const [hints, setHints] = useState<Record<number, string>>({});
    // shown: which hints are currently visible (toggled independently of cache)
    const [shown, setShown] = useState<Record<number, boolean>>({});
    const [loading, setLoading] = useState<Record<number, boolean>>({});
    const [errors, setErrors] = useState<Record<number, string>>({});

    const handleHint = async (clue: PlacedWord) => {
        // If already cached, just toggle visibility — no API call
        if (hints[clue.number]) {
            setShown(prev => ({ ...prev, [clue.number]: !prev[clue.number] }));
            return;
        }
        // Fetch and cache
        setLoading(prev => ({ ...prev, [clue.number]: true }));
        setErrors(prev => { const n = { ...prev }; delete n[clue.number]; return n; });
        try {
            const hint = await getHint(clue.answer, clue.clue);
            setHints(prev => ({ ...prev, [clue.number]: hint }));
            setShown(prev => ({ ...prev, [clue.number]: true }));
        } catch (err: any) {
            setErrors(prev => ({ ...prev, [clue.number]: err.message || 'Failed to load hint.' }));
        } finally {
            setLoading(prev => ({ ...prev, [clue.number]: false }));
        }
    };
    ...
    return (
        <div className="bg-white/70 backdrop-blur-xl border border-white p-6 rounded-3xl shadow-xl flex-1 max-h-[500px] overflow-y-auto">
            <h3 className="text-xl font-bold text-gray-800 mb-4 sticky top-0 z-10 bg-white/95 backdrop-blur-md pt-4 pb-3 -mt-4">{title}</h3>
            <ul className="space-y-3">
                {clues.map(c => (
                    <li key={c.number} className="text-sm">
                        <div className="flex gap-3 items-start">
                            <span className="font-bold text-indigo-600 w-6 text-right flex-shrink-0 pt-0.5">{c.number}.</span>
                            <div className="flex-1 min-w-0">
                                <span className="text-gray-700 leading-relaxed">{c.clue}</span>
                                <div className="mt-1">
                                    <button
                                        onClick={() => handleHint(c)}
                                        disabled={loading[c.number]}
                                        className="inline-flex items-center gap-1 px-2 py-0.5 text-xs font-semibold rounded-full bg-amber-100 text-amber-700 hover:bg-amber-200 transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
                                    >
                                        {loading[c.number] ? (
                                            <><span className="animate-spin inline-block">⟳</span> Loading…</>
                                        ) : (hints[c.number] && shown[c.number]) ? (
                                            <>💡 Hide Hint</>
                                        ) : (
                                            <>💡 Hint</>
                                        )}
                                ...
                                {errors[c.number] && (
                                    <p className="mt-1 text-xs text-red-500">{errors[c.number]}</p>
                                )}
                                {hints[c.number] && shown[c.number] && (
                                    <div className="mt-2 px-3 py-2 bg-amber-50 border border-amber-200 rounded-xl text-xs text-amber-800 leading-relaxed animate-in fade-in slide-in-from-top-1 duration-300">
                                        <span className="font-semibold">Hint: </span>{hints[c.number]}
                                    </div>
                                )}
                            </div>
                        </div>
